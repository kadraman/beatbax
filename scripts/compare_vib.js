...
function analyze(path){
  ...
  // detrend and compute autocorrelation of freq series to find vibrato rate
  ...
  // vibrato depth in cents: approximate by stddev of freq deviation
  const devs = detr.filter(v=>!isNaN(v));
  const std = Math.sqrt(devs.reduce((a,b)=>a+b*b,0)/devs.length);
  // depth in cents: 1200*log2((mean+std)/mean)
...
if (process.argv.length < 4) {
  console.log('Usage: node compare_vib.js <beatbax.wav> <huge.wav>'); process.exit(2);
}
