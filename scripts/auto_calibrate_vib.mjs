...
if (args.length < 1) {
  console.error('Usage: node scripts/auto_calibrate_vib.mjs <huge-ref.wav> [song.bax] [outDir]');
  process.exit(2);
}
const refWav = args[0];
...
// Parameter grid (coarse by default)
const vibDepthScales = [1, 2, 3, 4, 5];
const regBaseFactors = [0.04, 0.06, 0.08, 0.10, 0.12];
const regUnits = [1];
  ...
  for (const regBaseFactor of regBaseFactors) {
    for (const regUnit of regUnits) {
      ...
      try {
        ...
        // Extract depthCents values (A then B)
