...
import { materializeSequenceItems, normalizeSeqItems, patternEventsToTokens, } from '../structured.js';
...
function parseEnvelopeValue(v, vendorParam) {
    if (v == null)
        return null;
    // Already an object
    if (typeof v === 'object') {
        ...
        if (!Number.isNaN(level)) {
            ...
            const out = { level: Math.max(0, Math.min(15, Math.floor(level))), direction, period: Math.max(0, Math.floor(period)) };
            if (vendorParam)
                out.format = vendorParam;
            return out;
        }
        return null;
    }
    ...
    if (typeof v === 'string') {
        ...
        if (pref) {
            vendor = String(pref[1]).toLowerCase();
...
function parseNoiseValue(v, vendorParam) {
    if (v == null)
        return null;
    if (typeof v === 'object') {
    ...
    if (typeof v === 'string') {
        ...
        if (s.indexOf(',') >= 0) {
            ...
            const out = {};
            if (!Number.isNaN(clockShift))
                out.clockShift = clockShift;
            if (widthMode === 7 || widthMode === 15)
                out.widthMode = widthMode;
...
const warnProblematicPatternName = (name, loc) => {
...
const parseInstRhs = (name, rhs, insts, loc) => {
    const rest = rhs.trim();
    ...
    try {
        if (props.sweep) {
            ...
            if (typeof props.sweep === 'string') {
                ...
                if (m)
                    props.sweep = String(props.sweep).replace(/^[a-z]+:/i, '');
            }
            const parsed = parseSweep(props.sweep);
            if (parsed)
                props.sweep = parsed;
        }
    }
    ...
    if (isInstNormalizationEnabled()) {
        try {
            if (props.env) {
                ...
                const parsed = parseEnvelopeValue(props.env, vendor ?? undefined);
                if (parsed)
                    props.env = parsed;
            }
        }
        ...
        try {
            if (props.noise) {
                ...
                const parsed = parseNoiseValue(props.noise, vendor ?? undefined);
                if (parsed)
                    props.noise = parsed;
            }
        }
            ...
            if (props.width) {
                ...
                if (typeof props.width === 'string') {
                    ...
                    if (m) {
                        ...
                        if (!Number.isNaN(n))
                            props.width = { value: n, format: vf };
                    }
                    else if (vendor) {
                        const n = parseInt(s, 10);
                        if (!Number.isNaN(n))
                            props.width = { value: n, format: vendor };
                    }
                    else if (/^\d+$/.test(s)) {
...
const expandPatternSpec = (nameSpec, rhsRaw, rhsTokens, rhsEvents, loc) => {
...
const parseChannelRhs = (id, rhs, pats, loc) => {
    ...
    if (typeof ch.pat === 'string') {
        ...
        if (pats[base]) {
            ...
            if (res.instOverride)
                ch.inst = res.instOverride;
            ch.pat = res.tokens;
        }
    }
    return ch;
};
const parsePlay = (args) => {
...
const parseArrangeDefaults = (raw) => {
    if (!raw)
        return undefined;
    const out = {};
 ...
 */
function enhanceParseError(error, source) {
...
function suggestKeyword(word) {
    ...
    for (const kw of VALID_KEYWORDS) {
        ...
        if (!best || distance < best.distance)
            best = { keyword: kw, distance };
    }
    return best && best.distance <= 2 ? best.keyword : null;
}
function parseRecoveryError(stmt) {
...
export function parseWithPeggy(source) {
    ...
    const parseErrors = [];
    try {
        program = peggyParse(source, {});
        log.debug('Peggy parse complete', { statements: program.body.length });
    }
    catch (e) {
        const enhanced = enhanceParseError(e, source);
    ...
    let topTime = undefined;
    let topStepsPerBar = undefined;
    let chipName = undefined;
    let chipLoc = undefined;
    let topVolume = undefined;
    let playNode = undefined;
    let playLoc = undefined;
    const diagnostics = [];
    try {
        for (const stmt of program.body) {
            switch (stmt.nodeType) {
                ...
                case 'StepsPerBarStmt': {
                    topStepsPerBar = stmt.stepsPerBar;
                    break;
                }
                case 'VolumeStmt': {
                    topVolume = Math.max(0, Math.min(1, stmt.volume));
                    break;
                }
                case 'ChipStmt': {
                    chipName = stmt.chip;
                ...
                case 'EffectStmt': {
                    ...
                    const rhs = stmt.rhs ? String(stmt.rhs).trim() : '';
                    if (rhs)
                        effects[stmt.name] = rhs;
                    break;
                }
                case 'PatStmt': {
                    const { name, tokens } = expandPatternSpec(stmt.name, stmt.rhs, stmt.rhsTokens, stmt.rhsEvents, stmt.loc);
                ...
                case 'ArrangeStmt': {
                    ...
                    const parsedDefaults = parseArrangeDefaults(stmt.defaults ?? null);
                    arrs[stmt.name] = { name: stmt.name, arrangements: stmt.arrangements || [], defaults: parsedDefaults, loc: stmt.loc };
                    break;
                }
                case 'ChannelStmt': {
                ...
                case 'ErrorStmt': {
                    parseErrors.push(parseRecoveryError(stmt));
                    break;
                }
                default:
        ...
        const diag = (level, component, message, loc) => diagnostics.push({ level, component, message, loc });
        ...
        for (const ch of channels) {
            ...
            const hasSeqSpec = chAny.seqSpecTokens && chAny.seqSpecTokens.length > 0;
            ...
            if (!hasSeqSpec && !hasPat) {
                ...
                const rawTokens = hasSeqSpec ? chAny.seqSpecTokens : [];
                if (rawTokens.length === 0 && typeof ch.pat === 'string')
                    rawTokens.push(...ch.pat.split(/[\s,]+/));
        ...
        const isCallOrEffect = (v) => v.includes('(') || v.includes('<');
        if (patternEvents) {
            for (const [patName, events] of Object.entries(patternEvents)) {
                for (const ev of events) {
                    ...
                    if (val === '.' || val === '_' || val === '-')
                        continue;
                    if (insts[val] || pats[val] || seqs[val])
                        continue;
        ...
        const loc = toSourceLocation(e?.location ?? e?.loc);
        parseErrors.push({
    ...
    const ast = { pats, insts, seqs, channels, arranges: Object.keys(arrs).length ? arrs : undefined, bpm: topBpm, chip: chipName, volume: topVolume, play: playNode, metadata,
