...
function extractEffectType(effectPart) {
    if (!effectPart)
        return '';
    const parts = String(effectPart).split(':').map(s => s.trim()).filter(Boolean);
    if (parts.length >= 3)
        return parts[1].toLowerCase(); // e.g. gb:pan:L -> pan
    return parts[0].toLowerCase();
}
function mergeEffectsIntoToken(token, presetRhs) {
    if (!presetRhs || typeof token !== 'string')
        return token;
    const m = token.match(/^([^<]+)(<([^>]*)>)?$/);
    if (!m)
        return token;
    ...
    const keep = existingParts.slice();
...
export function applyModsToTokens(tokensIn, mods, presets, loc) {
    let tokens = tokensIn.slice();
    let semitones = 0;
    let octaves = 0;
    let instOverride = null;
    let panOverride = undefined;
    for (const mod of mods) {
        ...
        if (mInst) {
            instOverride = mInst[1];
            continue;
        }
        const mPan = mod.match(/^pan\(([^)]*)\)$/i);
        if (mPan) {
            panOverride = mPan[1].trim();
            continue;
        }
    ...
    if (instOverride)
        tokens.unshift(`inst(${instOverride})`);
    if (panOverride) {
        tokens.unshift(`pan(${panOverride})`);
        tokens.push(`pan()`);
    }
    return { tokens, instOverride, panOverride };
}
export function expandRefToTokens(itemRef, expandedSeqs, pats, presets, loc) {
...
//# sourceMappingURL=refExpander.js.map