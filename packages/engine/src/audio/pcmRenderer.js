 ...
 */
function renderWithPluginBackend(backend, freq, inst, buffer, startSample, durationSamples, sampleRate, numChannels, gains = { left: 1, right: 1 }) {
...
export function renderSongToPCM(song, opts = {}) {
    ...
    const pluginBackendsMap = new Map();
    if (!isGameBoy) {
        const plugin = chipRegistry.get(chipType);
        if (plugin) {
            for (let idx = 0; idx < plugin.channels; idx++) {
                try {
                    pluginBackendsMap.set(idx + 1, plugin.createChannel(idx, null));
                }
                catch (_) { }
            }
        }
    ...
    for (const ch of song.channels) {
        if (renderChannels.includes(ch.id)) {
            ...
            renderChannel(ch, instsClone, buffer, sampleRate, channels, tickSeconds, chipType, isGameBoy, vibDepthScale, regPerTrackerBaseFactor, regPerTrackerUnit, pluginBackend);
        }
    }
 ...
 */
function renderChannel(ch, insts, buffer, sampleRate, channels, tickSeconds, chipType, isGameBoy, vibDepthScale, regPerTrackerBaseFactor, regPerTrackerUnit, pluginBackend = null) {
    ...
    for (let i = 0; i < ch.events.length; i++) {
        ...
        if (pluginBackend && inst.type && inst.type.toLowerCase() === 'dmc') {
            ...
            if (extendedDur > dur)
                durationSamples = Math.floor(extendedDur * sampleRate);
        }
        if (ev.type === 'note') {
...
function renderNoteEvent(ev, inst, buffer, startSample, durationSamples, sampleRate, channels, tickSeconds, chipType, isGameBoy, vibDepthScale, regPerTrackerBaseFactor, regPerTrackerUnit, channelId, pluginBackend = null) {
    try {
 ...
 * @param channels Number of audio channels.
 */
function renderNamedEvent(ev, inst, buffer, startSample, durationSamples, sampleRate, channels, tickSeconds, isGameBoy, vibDepthScale, regPerTrackerBaseFactor, regPerTrackerUnit, channelId, pluginBackend = null) {
    // ── Plugin chip delegation ──────────────────────────────────────────────────
    if (!isGameBoy && pluginBackend) {
        const panSpec = resolveEventPan(ev, inst);
        const gains = panToGains(panSpec);
...
function renderWave(buffer, start, duration, freq, inst, sampleRate, channels, gains = { left: 1, right: 1 }, effects, tickSeconds, isGameBoy, vibDepthScale, regPerTrackerBaseFactor, regPerTrackerUnit, channelId, legato) {
    ...
    const waveTable = (_parsedWave && _parsedWave.length > 0) ? _parsedWave : DEFAULT_WAVE;
    // AC-coupling mean: computed once, used in the sample loop below.
    const waveMean = waveTable.reduce((a, b) => a + b, 0) / waveTable.length;
            ...
            try {
                if (fx && fx.type === 'vib') {
                    ...
                    if (typeof fx.durationSec === 'number')
                        ...
                        const durRows = Number(p[3]);
                        if (!Number.isNaN(durRows) && durRows > 0)
                            tremDurationSec = Math.max(0, Math.floor(durRows) * tickSeconds);
                    }
                    if (typeof fx.delaySec === 'number' && fx.delaySec > 0) {
                        tremDelaySec = fx.delaySec;
    ...
    if (typeof wave === 'string') {
        try {
            ...
            if (Array.isArray(parsed) && parsed.length > 0) {
                ...
                const tiled = [];
                for (let i = 0; i < GB_WAVE_LEN; i++)
                    tiled.push(mapped[i % mapped.length]);
                return tiled;
            }
        }
        catch (e) {
