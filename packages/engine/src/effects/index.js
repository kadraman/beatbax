...
register('vib', (ctx, nodes, params, start, dur) => {
    ...
    const waveformAliases = { sine: 'sine', square: 'square', triangle: 'triangle', sawtooth: 'sawtooth', saw: 'sawtooth', tri: 'triangle' };
    const waveformRaw = (Array.isArray(params) && typeof params[2] === 'string') ? params[2].toLowerCase().trim() : 'sine';
    const lfoType = (waveformAliases[waveformRaw] ?? 'sine');
    ...
    try {
        ...
        lfo.type = lfoType;
        try {
            lfo.frequency.setValueAtTime(rate, lfoStart);
        }
        catch (_) {
        ...
        if (hasFrequency) {
            ...
            if (!Number.isFinite(amplitudeHz) || amplitudeHz <= 0)
                return;
            try {
                lfoGain.gain.setValueAtTime(amplitudeHz, lfoStart);
            }
            catch (_) {
            ...
            const baseFreq = osc.__freq || 440;
            const baseRate = osc.__basePlaybackRate || osc.playbackRate.value;
            ...
            try {
                lfoGain.gain.setValueAtTime(amplitudeRate, lfoStart);
            }
            catch (_) {
                lfoGain.gain.value = amplitudeRate;
            }
            lfo.connect(lfoGain);
            try {
                lfoGain.connect(osc.playbackRate);
            }
            catch (e) {
                try {
                    lfoGain.connect(osc.playbackRate);
...
const portamentoLastGain = new Map();
 ...
 */
function computeLegatoEndGain(start15, env, dur) {
    if (!env || env.mode !== 'gb' || !env.period || env.period <= 0)
        return start15;
...
register('port', (ctx, nodes, params, start, dur, chId, _tickSeconds, inst) => {
    if (!nodes || nodes.length === 0)
        return;
    const osc = nodes[0];
    ...
    if (hasFreq) {
        ...
        let targetFreq = osc._baseFreq || osc.frequency.value;
        ...
        if (gainNode && gainNode.gain && inst) {
            ...
            gainNode.__portamento_legato__ = true;
            gainNode.__portamento_gain_start__ = Math.max(0.0001, tracked15 / 15);
            gainNode.__portamento_gain_end__ = Math.max(0.0001, end15 / 15);
        ...
        // __freq and __basePlaybackRate are attached by playWavetable in wave.ts
        const targetFreq = osc.__freq || 440;
        const targetRate = osc.__basePlaybackRate || osc.playbackRate.value;
        const lastFreq = portamentoLastFreq.get(channelKey) || targetFreq;
        if (Math.abs(targetFreq - lastFreq) > 1) {
    ...
    try {
        if (hasFrequency) {
            ...
            if (!Number.isFinite(baseRate) || baseRate <= 0)
                return;
            osc.playbackRate.cancelScheduledValues(start);
            const rates = allOffsets.map((offset) => baseRate * Math.pow(2, offset / 12));
            let currentTime = start;
            const endTime = start + dur;
            while (currentTime < endTime) {
...
register('trem', (ctx, nodes, params, start, dur) => {
    ...
    const src = nodes[0];
    const gain = nodes[1];
    if (!src || !gain || !gain.gain || typeof gain.gain.setValueAtTime !== 'function')
        return;
    ...
    const oscType = waveformAliases[waveform] || 'sine';
    ...
    try {
        ...
        try {
            tremGain.gain.setValueAtTime(1.0, start);
        }
        catch (_) {
