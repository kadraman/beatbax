...
function pushMetaText(data, delta, text) {
...
export async function exportMIDI(songOrPath, maybePath, options = {}, opts) {
    ...
    if (typeof songOrPath === 'string' && !maybePath) {
        ...
        if (opts && opts.debug)
            log.debug('Wrote empty MIDI to', outPath);
        return;
    }
    ...
    const channels = options.channels
        ? allChannels.filter((ch) => options.channels.includes(ch.id))
        : allChannels;
    ...
    function resolveProgramForInstrumentName(instName, ch, ev) {
        // priority: ev.instProps?.gm -> song.insts[instName]?.gm -> GB_TO_GM_PROGRAM based on type -> 0
        if (ev && ev.instProps && typeof ev.instProps.gm === 'number')
            return ev.instProps.gm & 0x7f;
        if (ev && ev.instProps && typeof ev.instProps.gm === 'string') {
            const parsed = parseInt(ev.instProps.gm, 10);
            if (!isNaN(parsed))
                return parsed & 0x7f;
        }
        if (instName && song && song.insts && song.insts[instName]) {
            const inst = song.insts[instName];
            if (inst && typeof inst.gm === 'number')
                return inst.gm & 0x7f;
            if (inst && typeof inst.gm === 'string') {
                const parsed = parseInt(inst.gm, 10);
                if (!isNaN(parsed))
                    return parsed & 0x7f;
            }
            if (inst && typeof inst.type === 'string') {
                const t = String(inst.type).toLowerCase();
                if (t.includes('pulse1'))
                    return GB_TO_GM_PROGRAM.pulse1;
                if (t.includes('pulse2'))
                    return GB_TO_GM_PROGRAM.pulse2;
                if (t.includes('wave'))
                    return GB_TO_GM_PROGRAM.wave;
                if (t.includes('noise'))
                    return GB_TO_GM_PROGRAM.noise;
            }
        }
    ...
    for (let ci = 0; ci < ntracks; ci++) {
        ...
        if (ch && ch.defaultInstrument && song && song.insts && song.insts[ch.defaultInstrument]) {
            ...
            if (tinst && typeof tinst.type === 'string' && String(tinst.type).toLowerCase().includes('noise'))
                midiChannel = 9;
        }
...
//# sourceMappingURL=midiExport.js.map