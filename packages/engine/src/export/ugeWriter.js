...
const VibratoHandler = {
    ...
    parse(fx, noteEvent, sustainCount, tickSeconds) {
        ...
        else if (fx.delaySec && Number.isFinite(fx.delaySec)) {
            delayRows = Math.max(0, Math.round(fx.delaySec / tickSeconds));
        }
...
function eventsToPatterns(events, instruments, channelType, dutyInsts, waveInsts, noiseInsts, strictGb = false, songBpm, desiredVibMap) {
    ...
    for (let i = 0; i < events.length; i++) {
        ...
        if (event.type === 'rest') {
            ...
            const noteSrc = namedEvent.defaultNote ?? namedInst?.note;
            if (noteSrc) {
    ...
    patterns.__hasTremEffects = hasTremEffects;
    return patterns;
}
/**
...
export async function exportUGE(song, outputPath, opts = {}) {
    ...
    if (opts && opts.debug)
        log.debug(`Discovered instruments: duty=${dutyInsts.length} wave=${waveInsts.length} noise=${noiseInsts.length}`);
    if (opts && opts.debug)
        log.debug(`Wave instrument names: ${JSON.stringify(waveInsts)}`);
    if (verbose) {
        ...
        if (dutyInsts.length > 0 || waveInsts.length > 0 || noiseInsts.length > 0) {
            ...
            if (dutyInsts.length > 0)
                log.info(`    - Duty: ${dutyInsts.length}/15 slots (${dutyInsts.join(', ')})`);
            if (waveInsts.length > 0)
                log.info(`    - Wave: ${waveInsts.length}/15 slots (${waveInsts.join(', ')})`);
            if (noiseInsts.length > 0)
                log.info(`    - Noise: ${noiseInsts.length}/15 slots (${noiseInsts.join(', ')})`);
        }
    }
    // ====== Instruments Section ======
    ...
    for (let i = 0; i < NUM_WAVE_INSTRUMENTS; i++) {
        if (i < waveInsts.length) {
            ...
            if (opts && opts.debug)
                log.debug(`Wave instrument '${name}' -> volume (beatbax)=${inst.volume ?? inst.vol ?? 'undefined'} ugeValue=${ugeVolume}`);
            ...
            if (inst.wave) {
                if (Array.isArray(inst.wave)) {
                    ...
                    if (/^[0-9A-Fa-f]{32}$/.test(hexStr)) {
                        waveData = hexStr.split('').map((c) => parseInt(c, 16));
                    }
                    else {
    ...
    for (let ch = 0; ch < NUM_CHANNELS; ch++) {
        ...
        if (opts && opts.debug)
            log.debug(`Channel ${ch + 1} has ${chEvents.length} events`);
    ...
    if (verbose) {
        ...
        if (vibCount > 0 || cutCount > 0) {
            ...
            if (vibCount > 0)
                log.info(`    - Vibrato: ${vibCount} note${vibCount !== 1 ? 's' : ''}`);
            if (cutCount > 0)
                log.info(`    - Note cuts: ${cutCount} occurrence${cutCount !== 1 ? 's' : ''}`);
        }
    }
        ...
        for (let row = 0; row < PATTERN_ROWS; row++) {
            ...
            if (nr51Value !== lastNr51 && hasNoteOn) {
                ...
                if (targetCell && (!targetCell.effectCode || targetCell.effectCode === 0)) {
                    ...
                    if (opts && opts.debug) {
                        try {
                            log.debug('[DEBUG] Skipping NR51 write due to existing effect on ch1 row', { orderIdx, row, existingEffect: targetCell && targetCell.effectCode });
                        }
                        catch (e) { }
                    }
                }
    ...
    try {
        if (opts && opts.debug) {
            try {
                log.debug('[DEBUG] finalEnforcement start', { nr51Writes: Array.from(nr51Writes.entries()), desiredVibMap: Array.from(desiredVibMap.entries()) });
            }
            catch (e) { }
        }
        for (let ch = 0; ch < NUM_CHANNELS; ch++) {
            ...
            for (let i = 0; i < chEvents.length; i++) {
                ...
                if (opts && opts.debug) {
                    try {
                        log.debug('[DEBUG] finalEnforce note check', { ch, globalStart, delayRowsEnforce, delayedStart, nrInfo, noteCellEffect: noteCell && noteCell.effectCode, preserveNoteNR51, allPatternsNoteCell: (allPatterns.find(p => p.channelIndex === ch && p.patternIndex === Math.floor(globalStart / PATTERN_ROWS)) || { cells: [] }).cells[globalStart % PATTERN_ROWS] });
                    }
                    catch (e) { }
                }
        ...
        if (opts && opts.debug) {
            log.debug('[DEBUG] final vib enforcement failed', e && e.stack ? e.stack : e);
        }
    }
    ...
    if (opts && opts.debug) {
        try {
            ...
            log.debug('[DEBUG] Pattern dump failed', e && e.stack ? e.stack : e);
        }
    ...
    if (opts && opts.debug)
        log.debug(`Total patterns: ${allPatterns.length}`);
    if (opts && opts.debug)
        log.debug(`Pattern breakdown: Ch1=${channelPatterns[0].length}, Ch2=${channelPatterns[1].length}, Ch3=${channelPatterns[2].length}, Ch4=${channelPatterns[3].length}`);
    ...
    for (let i = 0; i < allPatterns.length; i++) {
        ...
        if (ch >= 0 && ch < NUM_CHANNELS) {
            ...
            if (opts && opts.debug)
                log.debug(`Pattern ${i} for channel ${ch + 1}: ${nonEmpty.length} non-empty cells out of ${pattern.cells.length} total rows`);
            if (nonEmpty.length <= 20) {
                if (opts && opts.debug)
                    log.debug(`  Non-empty cells:`, nonEmpty.map((c) => {
