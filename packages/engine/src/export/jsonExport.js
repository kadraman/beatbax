 ...
 */
function validateSongModel(song) {
    const errors = [];
    if (!song || typeof song !== 'object')
        errors.push('song must be an object');
    // pats: record of name -> string[] (for AST or resolved model)
    if (!song.pats || typeof song.pats !== 'object')
        errors.push('missing or invalid `pats` object');
    else {
        for (const [k, v] of Object.entries(song.pats)) {
            if (!Array.isArray(v))
                errors.push(`pat '${k}' must be an array`);
            else if (v.length === 0)
                errors.push(`pat '${k}' is empty`);
            else {
                for (let i = 0; i < v.length; i++) {
                    if (typeof v[i] !== 'string')
                        errors.push(`pat '${k}' token at ${i} is not a string`);
                }
            }
        }
    }
    // insts: record of name -> props
    if (!song.insts || typeof song.insts !== 'object')
        errors.push('missing or invalid `insts` object');
    ...
    if (!Array.isArray(song.channels))
        ...
        for (const ch of song.channels) {
            if (typeof ch.id !== 'number')
                errors.push(`channel has invalid id: ${JSON.stringify(ch)}`);
            if (ch.inst && typeof ch.inst !== 'string')
                errors.push(`channel ${ch.id} inst must be a string`);
            // If channel.events is present (resolved ISM), validate events
            if (ch.events) {
                if (!Array.isArray(ch.events))
                    errors.push(`channel ${ch.id} events must be an array`);
                else {
                    for (const ev of ch.events) {
                        if (!ev || typeof ev !== 'object' || !ev.type)
                            errors.push(`channel ${ch.id} has invalid event ${JSON.stringify(ev)}`);
                    }
                }
            }
            else if (ch.pat) {
                if (typeof ch.pat === 'string') {
                    ...
                    for (let i = 0; i < ch.pat.length; i++) {
                        if (typeof ch.pat[i] !== 'string')
                            errors.push(`channel ${ch.id} pat token at ${i} is not a string`);
                    }
                }
            }
            if (ch.bpm !== undefined)
                errors.push(`channel ${ch.id} contains a top-level bpm which is not supported; use top-level 'bpm' or sequence transforms (fast/slow) instead`);
        }
    }
    if (errors.length > 0)
 ...
 */
export async function exportJSON(songOrPath, maybePath, opts) {
    let song = songOrPath;
    ...
    if (typeof songOrPath === 'string' && !maybePath) {
        ...
        if (opts && opts.debug)
            log.debug('Wrote JSON to', outPath);
        return;
    }
    if (!outPath)
        outPath = 'song.json';
    ...
    try {
        ...
        error('export', 'Validation error: ' + (err && err.message ? err.message : String(err)));
        throw err;
    }
    ...
    for (const ch of (clonedSong.channels || [])) {
        if (!Array.isArray(ch.events))
            continue;
        for (const ev of ch.events) {
            if (!ev || !Array.isArray(ev.effects) || ev.effects.length === 0)
                continue;
