...
export function expandSequenceItems(items, pats, insts, _missingWarned, presets, seqs, _seqVisiting) {
    const out = [];
    ...
    for (const it of itemStrs) {
        ...
        if (mGroup) {
            ...
            const visiting = _seqVisiting ?? new Set();
            if (visiting.has(realBase)) {
                ...
                tokens = expandSequenceItems(seqs[realBase], pats, insts, missingWarned, presets, seqs, visiting);
...
export function expandAllSequences(seqs, pats, insts, presets) {
    ...
    for (const [name, items] of Object.entries(seqs)) {
        ...
        const expanded = expandSequenceItems(items, pats, insts, undefined, presets, seqs);
        res[name] = expanded;
    }
    return res;
}
