...
import { normalizeRemoteUrl, validateRemoteUrl, } from './urlUtils.js';
/**
...
export class RemoteInstrumentCache {
    ...
    async fetchFromNetwork(url) {
        ...
        try {
            ...
            if (!response.ok) {
                throw new Error(`Failed to fetch remote instruments from ${url}: ${response.status} ${response.statusText}`);
            }
            ...
            if (contentLength) {
                ...
                if (size > this.options.maxFileSize) {
                    throw new Error(`Remote import file too large: ${size} bytes exceeds maximum of ${this.options.maxFileSize} bytes`);
                }
            }
            // Read response body
            ...
            if (actualSize > this.options.maxFileSize) {
                throw new Error(`Remote import file too large: ${actualSize} bytes exceeds maximum of ${this.options.maxFileSize} bytes`);
            }
            ...
            if (err instanceof Error && err.name === 'AbortError') {
                throw new Error(`Remote import timeout: failed to fetch ${url} within ${this.options.timeout}ms`);
            }
            throw err;
        }
        finally {
    ...
    validateInsFile(ast, url) {
        ...
        if (disallowed.length > 0) {
            throw new Error(`Invalid remote .ins file "${url}": remote .ins files may only contain "inst" declarations. ` +
