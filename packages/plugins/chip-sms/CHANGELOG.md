...
- bd3aeb2: Sync `src/version.ts` with `package.json` so SMS plugin version metadata reported by CLI/plugin surfaces stays accurate after releases.

## 0.3.4

### Patch Changes

- deb2069: Polish New Song Wizard chip metadata and starter demos across built-in and plugin chips.
