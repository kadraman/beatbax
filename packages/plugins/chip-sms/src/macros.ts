...
// Re-export the macro types and functions from the shared utility
// This is identical to the NES plugin's macros.ts
...
export function buildVolEnvGainCurve(
  ...
  for (let f = 0; f < totalFrames; f++) {
    // Convert vol_env value (0-15, 15=loudest) to gain multiplier (0.0-1.0)
    // by dividing by 15, matching the attenuation formula: vol/15 = 1 - (15-vol)/15
    vals.push((getMacroValue(macro, state) / 15) * mixGain);
