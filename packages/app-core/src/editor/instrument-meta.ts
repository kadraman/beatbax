...
export const CHIP_INSTRUMENT_META: Record<string, ChipInstrumentMeta> = {
  gameboy: {
    ...
    properties: {
      ...
      duty_env: { detail: 'Duty macro → UGE 9xx timbre, e.g. [2,1,0] (pulse)' },
      arp_env: { detail: 'Arp macro → UGE offsets when pitch_env is absent' },
      subpat: { detail: 'Named instrument subpattern, e.g. subpat=kick_body' },
  ...
  nes: {
    ...
    properties: {
      ...
      linear: { detail: 'Triangle linear counter gate length 0–127 (0 = sustain)' },
      noise_mode: { values: ['normal', 'loop'], detail: 'Noise LFSR mode' },
      noise_period: { detail: 'Noise period 0–15' },
      dmc_sample: { detail: 'DMC sample reference (@nes/, local:, https://, github:)' },
      dmc_rate: { detail: 'DMC playback rate 0–15' },
      dmc_loop: { values: ['true', 'false'], detail: 'Loop DMC sample' },
      dmc_level: { detail: 'Initial DMC DAC level 0–127' },
