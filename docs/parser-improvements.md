# Parser Improvements: Space Tolerance and Pattern Name Validation

## Overview

The BeatBax parser has been improved to be more forgiving and helpful when handling common user errors:
...
### Problem
Previously, the parser required `*` operators to be immediately adjacent to the closing `)` or token:
```bax
...
### Solution
The parser now normalizes spaces around `*` operators automatically. All of these formats now work correctly:
...
This also works for inline token repetition:
```bax
...
### Implementation
[`packages/engine/src/patterns/expand.ts`](packages/engine/src/patterns/expand.ts#L78-L81): The `expandPattern` function now includes preprocessing to normalize spacing:
...
In the sequence expander, if pattern `E` didn't exist in the patterns map, it would be treated as a literal token (note `E`), causing silent failures.

### Solution
The parser now warns when single-letter pattern names (A-G, case-insensitive) are used:
...
Single-letter names outside A-G (like `X`, `Y`, `Z`) do not trigger warnings, as they don't conflict with note names.
...
### Implementation
[`packages/engine/src/parser/index.ts`](packages/engine/src/parser/index.ts#L19-L24): Pattern name validation added during parsing:
...
Comprehensive tests added in [`packages/engine/tests/parser-space-tolerance.test.ts`](packages/engine/tests/parser-space-tolerance.test.ts):
...
- Related feature: [`docs/features/`](docs/features/) - pattern expansion spec
